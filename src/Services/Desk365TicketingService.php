...
class Desk365TicketingService implements TicketingServiceInterface
{
    ...
    {
        try {
            $endpoint = $this->getEndpoint('tickets/import_conversations', ['ticket_number' => $ticketNumber]);
            $response = $this->makeLoggedApiCall(
                method: 'POST',
                endpoint: $endpoint,
                headers: $this->config->getAuthHeaders(),
                data: $data->toArray(),
                timeout: $this->config->timeout,
                operation: 'importConversations'
            );

            return $this->handleResponse($response);
        } catch (\Exception $e) {
            Log::error('Desk365 API Error - Import Conversations', [
                'ticket_number' => $ticketNumber,
                'error' => $e->getMessage(),
            ]);

            return ApiResponseDto::error('Failed to import conversations: '.$e->getMessage());
        }
    }
